...
# v4a 只扩展 d31，d42 和 d42_31 保持 v3，方便单变量比较。
D31_RESIZE = "/model/decoder/d31/Resize"
D31_CONV = "/model/decoder/d31/conv_pre/conv_pre.0/Conv"
...
def find_cache_scale(
    ...
    nearby = sorted(
        ...
        if suffix in key or "d31" in key or "FEM3" in key
    )[:30]

    raise RuntimeError(
        "找不到 d31 Resize 输入张量的 calibration scale。\n"
        "tensor: {}\n"
        "tried: {}\n"
        "nearby cache keys:\n  {}".format(
            tensor_name,
...
def propagate_d31_qdq_to_resize(
    model: ModelProto,
    cache_scales: Dict[str, float],
) -> None:
    """
    在 d31 Resize 输入前插入 Q/DQ，保留 Resize 输出后的原 Q/DQ。

    修改前：
        raw -> Resize -> Q_out -> DQ_out -> d31 conv_pre

    修改后：
        raw -> Q_in -> DQ_in -> Resize -> Q_out -> DQ_out -> d31 conv_pre

    TensorRT 因而可以把位于 DQ_in 与 Q_out 之间的 Resize 作为
    INT8 层处理。d31 Add 仍在 conv_pre 输出后的高精度路径。
    """
    nodes_by_name, producer, consumers = build_node_maps(model)
    initializers = {init.name: init for init in model.graph.initializer}

    resize = nodes_by_name[D31_RESIZE]
    conv = nodes_by_name[D31_CONV]

    if resize.op_type != "Resize":
        raise RuntimeError("{} 不是 Resize".format(D31_RESIZE))
    if not resize.input or not resize.output:
        raise RuntimeError("d31 Resize 输入或输出为空")

    conv_input_dq = producer.get(conv.input[0])
    if conv_input_dq is None or conv_input_dq.op_type != "DequantizeLinear":
        raise RuntimeError("d31 conv_pre 输入不是 DQ")

    resize_output_q = producer.get(conv_input_dq.input[0])
    if resize_output_q is None or resize_output_q.op_type != "QuantizeLinear":
        raise RuntimeError("d31 conv_pre 前没有 Resize 输出 Q/DQ")

    if resize_output_q.input[0] != resize.output[0]:
        raise RuntimeError(
            "d31 Q/DQ 不直接位于 Resize 输出：Q input={}, Resize output={}".format(
    ...
    if existing_input_dq is not None and existing_input_dq.op_type == "DequantizeLinear":
        ...
        if existing_input_q is not None and existing_input_q.op_type == "QuantizeLinear":
            print("[KEEP] d31 Resize 输入前已经存在 Q/DQ")
            return
    ...
    if len(resize_output_q.input) < 2 or len(conv_input_dq.input) < 2:
        raise RuntimeError("现有 d31 输出 Q/DQ 缺少 scale 输入")

    scale_template = initializers.get(resize_output_q.input[1])
    if scale_template is None:
        raise RuntimeError(
            "找不到现有 d31 Q scale initializer: {}".format(
                resize_output_q.input[1]
            )
        )

    prefix = "/model/decoder/d31/Resize_input_v4"
    ...
    if len(resize_output_q.input) >= 3:
        ...
        if zp_template is None:
            raise RuntimeError(
                "找不到现有 d31 Q zero-point initializer: {}".format(
...
def validate_d31_resize_island(model: ModelProto) -> None:
    nodes_by_name, producer, consumers = build_node_maps(model)
    initializers = {init.name: init for init in model.graph.initializer}
    resize = nodes_by_name[D31_RESIZE]
    conv = nodes_by_name[D31_CONV]
    ...
    if (
        ...
        raise RuntimeError(
            "d31 Resize 输出 Q 后没有唯一专用 DQ: {}".format(
                [node.name for node in output_q_consumers]
            )
        )
    ...
    if [node.name for node in output_dq_consumers] != [conv.name]:
        raise RuntimeError(
            "d31 Resize 输出 DQ 未直接专供 conv_pre: {}".format(
    ...
    if input_scale is None:
        raise RuntimeError("d31 Resize 输入 Q 的 scale 不存在")
    ...
    if not np.isfinite(input_scale_value) or input_scale_value <= 0.0:
        raise RuntimeError("d31 Resize 输入 Q 的 scale 非法")

    print("\n[VALIDATION]")
    print("  d31 input chain : Q -> DQ -> Resize")
    print("  d31 output chain: Resize -> Q -> DQ -> conv_pre")
    print("  d31 input scale : {:.9g}".format(input_scale_value))


def validate_quantized_model(model: ModelProto) -> None:
    onnx.checker.check_model(model)
    validate_target_conv_qdq(model)
    validate_d31_resize_island(model)
    ...
    print("\n[PASS] d31 Resize 已进入显式 INT8 Q/DQ 岛")


def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "基于 v3 的 15-Conv Q/DQ 模型，将 d31 的 Q/DQ "
            "向上游扩展到 bilinear Resize。"
        )
    )
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        "--output",
        default="deploy/egcienet_352_multiclass_qdq_v4_d31_resize.onnx",
        help="最终显式 Q/DQ ONNX",
    )
    parser.add_argument(
        ...
        choices=("fp16", "fp32"),
        default="fp32",
        help="非 Q/DQ 区域的 ModelOpt 高精度类型；默认保持与 v3 一致",
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    parser.add_argument(
        "--keep-temporary",
        action="store_true",
        help="保留未经 d31 Resize 传播的 ModelOpt 中间模型",
    )
...
def main() -> None:
    ...
    require_nodes(source_model, [D31_RESIZE], "d31 Resize")
    ...
    require_nodes(quantized_model, [D31_RESIZE], "量化后的 d31 Resize")
    require_nodes(
        quantized_model,
        ENCODER_SHARED_CONSUMERS,
        "量化后的 Encoder 共享分支",
    ...
    propagate_d31_qdq_to_resize(quantized_model, cache_scales)
