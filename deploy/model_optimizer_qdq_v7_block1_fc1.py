...
"""
V7：在 V6 的基础上，仅额外量化 encoder block1 的 3 个 fc1/MatMul。

保留：
  - V5 的 12 个 FEM Conv + 3 个 GAM conv_pre Conv；
  - d31、d42、d42_31 三条 Resize 显式 INT8 Q/DQ 岛；
  - V6 的 block3.0~17 共 18 个 fc1/MatMul。

新增：
  - /model/block1.0~2/mlp/fc1/MatMul，共 3 个。
...
import model_optimizer_qdq_v6_block3_fc1 as v6


v5 = v6.v5

BLOCK1_FC1_TARGETS: List[str] = [
    "/model/block1.{}/mlp/fc1/MatMul".format(index)
    for index in range(3)
]

ALL_FC1_TARGETS: List[str] = [
    *BLOCK1_FC1_TARGETS,
...
def isolate_fc1_activation_qdq(
    model: ModelProto,
    target_names: Iterable[str],
) -> None:
    """
    保证每个目标 fc1 的激活 DQ 只服务于对应的 MatMul。

    如果 ModelOpt 把 DQ 输出共享给其他分支，让非目标消费者绕过 Q/DQ，
    ...
    print("\n[INFO] 隔离 {} 个 fc1/MatMul 的激活 Q/DQ".format(len(targets)))
...
def validate_fc1_qdq(model: ModelProto) -> None:
    """严格检查只有 block1 + block3 的 21 个 fc1 MatMul 被量化。"""
    nodes_by_name, producer, consumers = v5.build_node_maps(model)
    expected: Set[str] = set(ALL_FC1_TARGETS)
    ...
    for target_name in ALL_FC1_TARGETS:
        matmul = nodes_by_name[target_name]
        activation_dq = producer.get(matmul.input[0])
    ...
    if missing:
        raise RuntimeError("目标 fc1 未量化: {}".format(missing))
    if unexpected:
        raise RuntimeError(
    ...
    if activation_dq_errors:
        raise RuntimeError(
            "目标 fc1 的激活 DQ 非专用: {}".format(activation_dq_errors)
        )
...
def validate_quantized_model(model: ModelProto) -> None:
    ...
    # V5 校验保证只有原来的 15 个 Decoder Conv 被量化；DWConv 一旦进入
    # 权重量化集合会作为 unexpected Conv 直接报错。
    v5.validate_target_conv_qdq(model)
    validate_fc1_qdq(model)
    ...
    print("\n[PASS] 仅新增 block1 fc1 Q/DQ；fc2、attention 和 DWConv 未被量化")


def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "基于 V6，只额外量化 block1 的 3 个 fc1/MatMul；"
            "保留 block3 的 18 个 fc1，不量化 fc2、attention 和 DWConv。"
        )
    )
    parser.add_argument(
        "--input",
...
def main() -> None:
    ...
    print("[TARGET] 15 Decoder Conv + 3 block1 fc1 + 18 block3 fc1")
    ...
    v5.require_nodes(source_model, BLOCK1_FC1_TARGETS, "目标 block1 fc1 MatMul")
    v5.require_nodes(
        source_model,
        v6.BLOCK3_FC1_TARGETS,
    ...
    isolate_fc1_activation_qdq(quantized_model, ALL_FC1_TARGETS)
